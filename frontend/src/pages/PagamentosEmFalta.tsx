...
import { clientService, paymentsPendingReportService, PaymentsPendingRow } from '../api/apiService';
import DownloadIcon from '@mui/icons-material/Download';
import FileDownloadOutlinedIcon from '@mui/icons-material/FileDownloadOutlined';

const PagamentosEmFalta = () => {
  const [clients, setClients] = useState<any[]>([]);
  const [selectedClient, setSelectedClient] = useState<string>('all');
  const [rows, setRows] = useState<PaymentsPendingRow[]>([]);
  ...
  const fetchReport = async (clientId?: string) => {
    ...
    setError('');
    try {
      const data = await paymentsPendingReportService.getReport({
      ...
      setError('Erro ao carregar pagamentos em falta');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 4 } }}>
      ...
      <Typography variant="subtitle1" gutterBottom>
        Valores à data de {new Date(reportDate).toLocaleDateString('pt-PT')}
      </Typography>
        ...
        <Table>
          ...
          <TableBody>
            ...
            <TableRow>
              ...
              <TableCell>
                <strong>
                  {totals.outstanding.toLocaleString('pt-PT', {
                    style: 'currency',
                    currency: 'AOA',
                    minimumFractionDigits: 2,
                  })}
