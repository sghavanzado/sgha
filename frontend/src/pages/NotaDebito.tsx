...
import { clientService, debitNoteService } from '../api/apiService';
...
const NotaDebito = () => {
  ...
  useEffect(() => {
    const initializeDebitNote = async () => {
      try {
        ...
        const nextNumber = await debitNoteService.getNextDebitNoteNumber();
        setFormData(prev => ({
          ...prev,
          debit_note_number: nextNumber.debit_note_number
        }));
      } catch (error) {
        setLoading(prev => ({ ...prev, clients: false }));
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError('Error al guardar la nota de débito.');
    } finally {
